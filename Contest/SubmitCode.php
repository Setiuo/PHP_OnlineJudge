...
if (isset($_POST["code"]) && isset($_POST["language"]) && isset($_POST["ConID"]) && isset($_POST["NowPro"])) {
	...
	//向数据库中插入状态
	$sql = 'INSERT INTO oj_constatus(`ConID`, `User`, `Problem`, `Status`, `UseTime`, `UseMemory`, `Language`, `CodeLen`, `SubTime`, `AllStatus`, `Show`) values(' . $ConID . ', "' . $LandUser . '", ' . $ConProblemID . ', ' . Wating . ', -1, -1, "' . $Language . '", ' . $CodeLen . ', "' . $NowTime . '", " ", 1)';
